...
public class BevasarloListaController {
    ...
    List<Keszlet> alapanyagList = KeszletController.getInstance().Osszes();

    public List<String> alapanyagNevek(List<Keszlet> k){
        List<String> result = new ArrayList<>();
        for(int next = 0; next <= k.size()-1; next++){
            result.add(k.get(next).getNev());
        }
        return result;
    }
    ...
    public void initialize(){
        ...
        mertekegysegCh.getItems().addAll("kg", "dkg", "g", "db", "evőkanál", "teáskanál", "kávéskanál", "db", "l", "dl", "csipet");
    }
